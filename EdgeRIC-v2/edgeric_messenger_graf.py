...
class EdgericMessenger:
    def __init__(self, socket_type):
        ...
        self.subscriber.connect("tcp://localhost:5555")  # Connect to the IPC address for metrics (5551)
        ...
        if socket_type == "weights":
            ...
            self.publisher_socket.bind("tcp://localhost:5556")  # Bind to the IPC address for weights (5552)
        elif socket_type == "mcs":
            # Create a publisher socket for MCS
            self.publisher_socket = self.context.socket(zmq.PUB)
            self.publisher_socket.bind("tcp://localhost:5557")  # Bind to the IPC address for MCS (5553)
    ...
    def update_prometheus_metrics(self):
        while True:
            try:
                ...
                ue_dict = self.ue_dict #self.get_metrics(flag_print=True)
