...
def main(conf):
    ...
    while flag_violations:
     for traj in range(1):
        ...
        while not done:
            ...
            for i in range(len(RNTIs)):
                ...
                avg_rewards[ue] = avg_rewards[ue]*(1-gamma) + reward[ue]*(gamma)
                if algo == "neurwin":
                    if BLs[i] == 0: 
                        indices[ue] = -100
                    else: 
                        ...
                        if ue == 0:
                            indices[ue] = indices[ue]*0.05
                        if ue == 2:
                            indices[ue] = indices[ue]*0.08
                if algo == "maxwt":
                    ...
                    if avg_rewards[ue] != 0:
                        indices[ue] = CQIs[i]*(B[ue]/5000)/(avg_rewards[ue])#BL*CQI 
                    else:
                        indices[ue] = 100
                elif algo == "maxCQI":  
                    #indices[ue] = CQIs[i]
                    if avg_rewards[ue] != 0:
                            # indices[ue] = QCIP[env[ue].app]*(B[env[ue].app]/5000)/(episode_reward[ue]/(time_rr+1))#BL*CQI 
                            # indices[ue] = QCIP[env[ue].app]/(episode_reward[ue])#BL*CQI 
                        indices[ue] = 1/(QCIP[ue]*(avg_rewards[ue]))#BL*CQI
                    else:
                        indices[ue] = 100
     ...
     for ue in range(NUM_UES):
        ...
        violation_tpt_dict[f"UE{ue}"].append(violation_tpt[ue])
        violation_tsls_dict[f"UE{ue}"].append(violation_tsls[ue])
        # if algo == "neurwin":
        #  if ue == 0 or ue == 2:
        #     if violation_tpt[ue] <= 0:
        #         t = t + 1
                
        #     elif violation_tpt[ue] >= 0:
        #         if ue == 0 or ue == 3:
        #             a = 0
        #         else:
        #             a = 0 #0.001
        #         violation_tpt_picked[ue] = violation_tpt_picked[ue] + a*violation_tpt[ue]/(p[ue]*B[ue])
                    
        #  if ue == 1:
        #         b = 50
        #  else:
        #         b = 3
        #  if violation_tsls[ue] <= b:
        #         t = t + 1
                
        #  elif violation_tsls[ue] >= b:
        #     if ue == 0 or ue == 3:
        #         a = 0
        #     elif ue == 1:
        #         a = 0.01
        #     violation_tsls_picked[ue] = violation_tsls_picked[ue] + a*violation_tsls[ue]/episodic_tsls[ue]
        #  if t == 40:
        #     flag_violations = False
        # else:
        #     flag_violations = False
    ...
    #     file.write(f"TPT_Violation_Picked: {violation_tpt_picked}\n")
    #     file.write(f"TPT_Violation_Picked: {violation_tsls_picked}\n")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--urllcport', type=int, default=1234, help='Port Number')
    parser.add_argument('--xrport', type=int, default=12345, help='Port Number')
    args = parser.parse_args()

    loaders = []
    ue_dict = {0: 17921, 1: 17924, 2: 17923, 3: 17922}
    ...
    p = {0: 0.80, 1: 0.80, 2: 0.80, 3: 0.80}
    B = {0:124215.275, 1: 617480.725, 2: 1542542.0, 3: 0} #trace1
   # B = {0:786274.25, 1:168586.125, 2:1546675.875, 3:0}
    QCIP = {0: 70, 1: 55, 2: 68, 3: 100}
    ...
    for ue in range(NUM_UES):
        ...
        index_tsls[ue] = 0
        if ue == 0:
            violation_tpt_picked[ue] = 0 #0.5  
            violation_tsls_picked[ue] =  0 #0.5
        if ue == 1:
            violation_tpt_picked[ue] = 1.0
            violation_tsls_picked[ue] = 0.9 #0.61161778
        if ue == 2:
            violation_tpt_picked[ue] =  2.6184214108919063e-05  
            violation_tsls_picked[ue] = 0 #0.5 #0.008559453333
        if ue == 3:
    ...
    for ue in range(NUM_UES):
        ...
        episodic_tsls[ue] = 0.0
    results = [] 
    algos = ["maxwt", "maxCQI", "propfair", "roundrobin", "neurwin"]
    violation_tpt_dict = {}
    violation_tsls_dict = {}
    tpt_moving_avg = {}
    tsls_moving_avg = {}
    for ue in range(NUM_UES):
        violation_tpt_dict[ue] = []
        violation_tsls_dict[ue] = []
        tsls_moving_avg[ue] = []
        tpt_moving_avg[ue] = []
    ...
    while True:
     for algorithm in algos:
        ...
        window_size = 1
        for ue in range(NUM_UES):
            current_sum = sum(violation_tpt_dict[ue]/window_size)
            tpt_moving_avg[ue].append(current_sum / window_size)
            current_sum = sum(violation_tsls_dict[ue]/window_size)
            tsls_moving_avg[ue].append(current_sum / window_size)

        ##### funct animation that shows tpt_moving_avg and tsls_moving_avg#####
